...
def update_stock(sku):
    ...
    for product_desc in sku[:count_sku]:
        ...
        jakpus = int(product_desc["Toko Jakarta Pusat"])
        jakbar = int(product_desc["Toko Jakarta Barat"])

        if (jakpus != 0) or (jakbar != 0):
...
def validation(response, data, product_desc, sku_state):
    ...
    if validation:
        ...
        for bit in data:
            ...
            if validation_x:
                print(f"{response_x.text}\n")

                skuID = product_desc["SKU"]
                productID = sku_dict.get(skuID)
                print(f"Error msg from SKU {skuID}: {productID}\n")

            if validation_x == "ERROR_VALIDATION":
                ...
                file_py_path = f"sku_product_mapping{file_name}.py"

                with open(file_py_path, "w") as f:
                    f.write(f"sku_product_mapping = [\n")
                    f.write("    {\n")
